...
public class TalkInformationData : ScriptableObject // フレンドの個人チャットorグループを統括するデータ
{
    ...
    public TalkInformationData Copy()
    {
        ...
        if (questionData != null) copy.questionData = questionData.Copy();

        copy.replyDataList = new List<ReplyData>();
        if (replyDataList != null) foreach (ReplyData replyData in replyDataList) copy.replyDataList.Add(replyData.Copy());

        copy.defaultQuestionReply = new List<BaseEventData>();
        if (defaultQuestionReply != null) foreach (BaseEventData eventData in defaultQuestionReply) copy.defaultQuestionReply.Add(eventData.Copy());

        copy.defaultReply = new List<BaseEventData>();
        if (defaultReply != null) foreach (BaseEventData eventData in defaultReply) copy.defaultReply.Add(eventData.Copy());
